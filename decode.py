...
class BeamSearchDecoder:
    ...
    def __init__(self, model, batcher, vocab):
        ...
        if FLAGS.single_pass:
            ...
            self._decoder_dir = os.path.join(FLAGS.log_root, get_decoder_dir_name(ckpt_name))
            if os.path.exists(self._decoder_dir):
                raise Exception('Single_pass decode directory %s should not already exist'%self._decoder_dir)

        else:
            self._decode_dir = os.path.join(FLAGS.log_root, 'decode')

        if not os.path.exists(self._decode_dir):os.mkdir(self._decoder_dir)
...
def get_decoder_dir_name(ckpt_name):
